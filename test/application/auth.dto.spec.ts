...
describe('AuthDto Validation', () => {
  ...
  describe('Password', () => {
    ...
    it('dovrebbe fallire se manca la lettera maiuscola', async () => {
      ...
      expect(Object.values(errors[0].constraints!)).toContain('Password must contain at least one uppercase letter');
    });
    ...
    it('dovrebbe fallire se manca la lettera minuscola', async () => {
      ...
      expect(errors.length).toBeGreaterThan(0);
      expect(errors[0].constraints).toHaveProperty('matches');
      expect(Object.values(errors[0].constraints!)).toContain('Password must contain at least one lowercase letter');
    });
    ...
    it('dovrebbe fallire se manca il numero', async () => {
      ...
      expect(errors.length).toBeGreaterThan(0);
      expect(errors[0].constraints).toHaveProperty('matches');
      expect(Object.values(errors[0].constraints!)).toContain('Password must contain at least one number');
    });
    ...
    it('dovrebbe fallire se manca il carattere speciale', async () => {
      ...
      expect(errors.length).toBeGreaterThan(0);
      expect(errors[0].constraints).toHaveProperty('matches');
      expect(Object.values(errors[0].constraints!)).toContain('Password must contain at least one special character');
    });
  });
});